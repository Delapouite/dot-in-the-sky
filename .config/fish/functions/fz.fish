function fz --description 'entry point for all the fuzziness glory'
	...
	function json_preview
		printf "--preview\n$argv[1] | bat --plain --language json --color always"
	end
	...
	case 'azure-*'
		...
		case azure-accounts
			...
			set --local choice (az account list \
				...
				| _awk "$awk_dim4" \
				| _fzf (json_preview 'az account show --subscription {-1}') \
					--accept-nth -1)
		...
		case azure-appservice-functionapps
			...
			set --local choice (az functionapp list \
				...
				| _fzfa \
					--header "$account" \
					(json_preview 'az functionapp show --name {1} --resource-group {2}') \
					--accept-nth 1)
		...
		case azure-appservice-plans
			...
			az appservice plan list \
				...
				| _fzf \
					--header "$account" \
					(json_preview 'az appservice plan show --name {1} --resource-group {2}')
		...
		case azure-appservice-webapps
			...
			az webapp list \
				...
				| _fzf \
					--header "$account" \
					(json_preview 'az webapp show --name {1} --resource-group {2}')
		...
		case azure-container-registries
			...
			set --local choice (az acr list \
				...
				| _fzf \
					--header "$account" \
					(json_preview 'az acr show --name {1}')
					--accept-nth 2)
		...
		case azure-container-registry-manifests
			...
			az acr manifest list-metadata "$registry/$repository" --orderby time_desc 2> /dev/null \
				...
				| _fzf --query '' \
					...
					--header "$account" \
					(json_preview "az acr manifest show-metadata $registry/$repository@{1} 2> /dev/null") \
					--accept-nth 1
		...
		case azure-container-registry-repositories
			...
			set --local choice (az acr repository list 2> /dev/null \
				...
				| _fzf --query '' \
					...
					--header "$account" \
					(json_preview 'az acr repository show --repository {1} 2> /dev/null') \
					--accept-nth 1)
		...
		case azure-event-hub-namespaces
			...
			set --local choice (az eventhubs namespace list --resource-group "$rg" \
				...
				| _fzf \
					--header "$account" \
					--expect ^ \
					(json_preview 'az eventhubs namespace show --name {1}'))
		...
		case azure-event-hubs
			...
			set --local choice (az eventhubs eventhub list --resource-group "$rg" --namespace-name "$iothub" \
				...
				| _fzf \
					...
					--header "$account" \
					--expect ^ \
					(json_preview 'az eventhubs eventhub show --name {1}'))
		...
		case azure-extensions
			...
			az extension list-available \
				...
				| _fzf \
					--header "$account" \
					--expect ^ \
					(json_preview 'az extension show --name {1}')
		...
		case azure-iot-dps
			...
			set --local choice (az iot dps list \
				...
				| _fzf \
					...
					--expect ctrl-alt-c \
					--expect ctrl-alt-g \
					(json_preview 'az iot dps show --name {1}'))
		...
		case azure-iot-dps-certificates
			...
			set --local choice (az iot dps certificate list --dps-name "$iotdps" \
				...
				| _fzf \
					...
					--header "$account" \
					--expect ^ \
					(json_preview "az iot dps certificate show --dps-name $iotdps --certificate-name {1}"))
		...
		case azure-iot-dps-groups
			...
			set --local choice (az iot dps enrollment-group list --dps-name "$iotdps" \
				...
				| _fzf \
					...
					--header "$account" \
					--expect ^ \
					(json_preview "az iot dps enrollment-group show --dps-name $iotdps --group-id {1}"))
		...
		case azure-iot-hubs
			...
			set --local choice (az iot hub list \
				...
				| _fzf \
					...
					--expect ctrl-alt-e \
					--expect ctrl-alt-r \
					(json_preview 'az iot hub show --name {1}'))
		...
		case azure-iot-hub-endpoints
			...
			set --local choice (az iot hub message-endpoint list --hub-name "$iothub" \
				...
				| _fzf \
					...
					--expect ^ \
					--expect ctrl-alt-r \
					(json_preview "az iot hub message-endpoint show --hub-name $iothub --endpoint-name {1}"))
		...
		case azure-iot-hub-routes
			...
			set --local choice (az iot hub message-route list --hub-name "$iothub" \
				...
				| _fzf \
					...
					--expect ^ \
					--expect ctrl-alt-e \
					(json_preview "az iot hub message-route show --hub-name $iothub --route-name {1}"))
		...
		case azure-resources
			...
			set --local choice (az resource list \
				...
				| _fzf \
					--header "$account" \
					--expect ^ \
					(json_preview "az resource show --name {1} --resource-type {-2} --resource-group {-1}"))
		...
		case azure-storage-accounts
			...
			set --local choice (az storage account list \
				...
				| _fzf \
					--header "$account" \
					--expect ^ \
					(json_preview 'az storage account show --name {1}'))
		...
		case azure-storage-blobs
			...
			set --local choice (az storage blob list --account-name "$storageaccount" --container-name "$container" 2> /dev/null \
				...
				| _fzf --query '' \
					...
					--expect ^ \
					--expect enter \
					(json_preview "az storage blob show --account-name="$storageaccount" --container-name='$container' --name {1} 2> /dev/null"))
		...
		case azure-storage-containers
			...
			set --local choice (az storage container list --auth-mode login --account-name "$storageaccount" 2> /dev/null \
				...
				| _fzf --query '' \
					...
					--expect ^ \
					(json_preview "az storage container show --account-name="$storageaccount" --name {1} 2> /dev/null"))
	...
	case 'docker-*'
		...
		case docker-containers
			...
			docker containers -a \
				| _fzfh (json_preview 'docker container inspect {1} | jq .[0]')
		...
		case docker-images
			...
			docker images \
				| _fzfh (json_preview 'docker image inspect {3} | jq .[0]')
		...
		case docker-networks
			...
			docker networks \
				| _fzfh (json_preview 'docker network inspect {1} | jq .[0]')
		...
		case docker-registries
			...
			_jq '.auths | keys[]' ~/.docker/config.json | _fzf
		...
		case docker-volumes
			...
			docker volumes \
				| _fzfh (json_preview 'docker volume inspect {3} | jq .[0]')
	...
	case 'podman-*'
		...
		case podman-volumes
			...
			podman volumes \
				| _fzfh (json_preview 'podman volume inspect {3} | jq .[0]')
