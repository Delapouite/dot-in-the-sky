...
(cl-defun my/parse-stack-response (&key data &allow-other-keys)
  (progn
    ...
    (let ((item (aref (assoc-default 'items data) 0)))
      ...
      (org-set-property "score" (number-to-string (assoc-default 'score item)))
      (org-set-property "views" (number-to-string (assoc-default 'view_count item)))
      ...
      :success (cl-function
                (lambda (&key data &allow-other-keys)
                  ...
                  (let ((item (aref (assoc-default 'results data) 0)))
                    (org-set-property "name" (assoc-default 'Name item))
                    (org-set-property "description" (assoc-default 'Description item))
                    (org-set-property "remote-version" (assoc-default 'Version item))
                    (org-set-property "system-version" (string-trim (shell-command-to-string (concat "pacman -Q " package-id " 2> /dev/null | awk '{ print $2 }'"))))
