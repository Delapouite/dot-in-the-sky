  ...
  (seq-let (username) (my/parse-url my/github-user-re)
    (my/fetch
     ...
     (lambda (data)
       ...
       (org-set-property "bio" (s-replace "\r\n" "" (or (assoc-default 'bio data) "null")))
       (my/org-set-prop "followers" 'followers data)
       (my/org-set-prop "following" 'following data)
       (org-set-property "author" (concat "https://github.com/search?q=author:" username))
       (org-set-property "commenter" (concat "https://github.com/search?q=commenter:" username))
       (my/org-set-prop "created-at" 'created_at data)
       (my/org-set-prop "updated-at" 'updated_at data)
       (my/fetched-at)))))
    ...
    (my/fetch
     ...
     (lambda (data)
       ...
       (letrec ((item (aref (assoc-default 'items data) 0))
                ...
                (videos (my/assoc-default item 'statistics 'videoCount)))
         ...
         (org-set-property "videos" videos)
         ...
         (my/fetch
          ...
          (lambda (data)
            (when (> (string-to-number videos) 0)
              (let ((item (aref (assoc-default 'items data) 0)))
